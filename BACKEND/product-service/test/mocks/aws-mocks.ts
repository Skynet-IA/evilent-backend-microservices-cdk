 ...
 * ✅ Respuestas incluyen: statusCode + headers + body JSON
 */
 ...
 * REGLA CRÍTICA: Validar estructura exacta con statusCode + headers + body
 */
 ...
 * Estructura: { success: true, message: string, data?: unknown }
 * REGLA CRÍTICA: Incluye headers como en response.ts
 */
 ...
 * Estructura: { success: false, message: string, data?: unknown }
 * REGLA CRÍTICA: Incluye headers como en response.ts
 */
 ...
 * Estructura: { success: false, message: string, data: { errors: [...] } }
 * REGLA CRÍTICA: Coincidir exactamente con ValidationErrorResponse de response.ts
 */
...
/**
 * Mock de S3Client
 * Usado en image-handler para upload de archivos
 */
...
export const mockGetSignedUrl = jest
  .fn()
  .mockResolvedValue('https://mock-signed-url.s3.amazonaws.com/test-image.jpg');
